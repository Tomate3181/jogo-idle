...
class Player extends Phaser.GameObjects.Sprite {
  constructor(scene, x, y, savedData = {}) {
    ...
    if (this.body) {
      // Boa prática: verificar se o body existe
    ...
    this.coinLevel =
      savedData.coinLevel !== undefined ? savedData.coinLevel : 1;
    this.magnetLevel =
      savedData.magnetLevel !== undefined ? savedData.magnetLevel : 0; // Ímã é um upgrade
  ...
  takeDamageFromEnemy(player, enemy) {
    ...
    if (this.scene.time.now > player.lastHitTime + config.PLAYER_HIT_COOLDOWN) {
      ...
      if (player.health <= 0) {
        player.health = 0;
        this.emit("player-died"); // agora é emitido no player
      }
    }
  }
