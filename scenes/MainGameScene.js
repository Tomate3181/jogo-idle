...
class MainGameScene extends Phaser.Scene {
    ...
    create() {
        ...
        const savedData = this.configManager.loadGame();
        ...
        // Player
        this.player = new Player(this, config.GAME_WIDTH / 2 - 100, config.GAME_HEIGHT / 2, savedData);
        ...
        const basicPistol = WeaponFactory.createWeapon(this, this.player, this.bullets, 'pistol', WeaponFactory.getWeaponVariations('pistol')[0]);
        const basicSword = WeaponFactory.createWeapon(this, this.player, this.bullets, 'sword', WeaponFactory.getWeaponVariations('sword')[0]);
        ...
        this.hud = new HUD(this, this.player, ['pistol', 'sword'], 'pistol', savedData);
        ...
        this.configManager.init(this.player, this.hud, this.enemies, this.bullets, this.enemyBullets);

        // Spawn inicial de moedas
        this.spawnInitialCoins();

        // Input para troca de armas
        this.input.keyboard.on('keydown-ONE', () => { this.equipWeapon('pistol'); });
        this.input.keyboard.on('keydown-TWO', () => { this.equipWeapon('sword'); });

        // Pega o ponteiro do mouse
        this.pointer = this.input.activePointer;

        // --- Adição de colisões e overlaps ---
        this.physics.add.overlap(this.player, this.coins, this.player.collectCoin, null, this.player);
        this.physics.add.overlap(this.player, this.enemies, this.player.takeDamageFromEnemy, null, this.player);
        ...
        this.physics.add.overlap(this.player, this.configManager.dropCrates, this.configManager.openCrate, null, this.configManager);


        // Timer para salvar o jogo periodicamente (ConfigManager faz o save)
        this.time.addEvent({
            delay: 5000,
            callback: () => this.configManager.saveGame({
    ...
    handleBulletHitEnemy(bullet, enemy) {
        if (bullet.active && enemy.active) {
            ...
            if (!enemy.active) {
                ...
                this.player.addScore(config.COIN_POINTS_VALUE * 2 + this.configManager.currentWave * 2);
    ...
    restartGame() {
        ...
        // Destrói timers e eventos existentes para evitar duplicação
        this.configManager.destroy(); // Destrói timers do configManager
        this.time.clearPendingEvents();

        // Limpa o player.on('player-died') para evitar múltiplos listeners
        if (this.player) {
            this.player.off('player-died', this.restartGame, this);
        }

        // Reseta todos os estados do jogo para o padrão
        this.player.reset();
        this.shop.reset();
        this.hud.reset();
        this.configManager.reset(); // Reseta variáveis de onda/mundo e timers

        // Destrói inimigos, balas, moedas e baús existentes
        this.enemies.clear(true, true);
        this.bullets.clear(true, true);
        this.enemyBullets.clear(true, true);
        this.coins.clear(true, true);
        this.configManager.dropCrates.clear(true, true);

        // Limpa o savegame no local storage (ou carrega um estado inicial limpo)
        localStorage.removeItem('idleGameSave');

        // Reinicia a cena para recriar todos os objetos com estados iniciais
        this.scene.restart();
    }
}
